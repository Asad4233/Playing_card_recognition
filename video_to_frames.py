...
videos = [
    os.path.join("Dataset", video) for video in os.listdir("Dataset") if ".mp4" in video
]
...
for video in tqdm(videos):
    ...
    print("", end="\n")
    print(
        f"Video duration is {duration :.2f} seconds and is recorded at {fps :.2f} FPS."
    )
    ...
    while video.isOpened():
        ...
        if ret:
            ...
            if h < w:
                ...
                frame = cv2.resize(
                    frame, (w // scale, h // scale), interpolation=cv2.INTER_LINEAR
                )
            elif h > w:
                frame = cv2.resize(
                    frame, (h // scale, w // scale), interpolation=cv2.INTER_LINEAR
                )
